...
**题目1：给定一个循环数组$a_1,\ldots,a_n$，要求将数组分成若干个连续子数组，且每个子数组的总和不能超过$k$，要求找到最少可以切分为多少个子数组。其中$0\leq a_i\leq k$。**

如果我们知道任意一次切分的起点，那么我们可以很容易用贪心算法找出最少划分次数。为了避免循环数组带来的麻烦，我们把数组扩展到长度$2n$，其中$a_{i}=a_{i+n}$。如果从$a_i$，最多贪心到$a_j$，则从$i$向$j+1$连一条有向边。

我们如果以$i$作为划分点，那么最少需要的划分数等价于从$i$出发沿着有向边移动直到抵达编号大于等于$i+n$的点所经过的最少边数。

我们可以通过倍增，之后对于每个划分点，我们可以$O(\log_2n)$时间复杂度内求出最少的划分次数。总的时间复杂度为$O(n\log_2n)$。