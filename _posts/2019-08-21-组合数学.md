...
**题目2：现在有$n$个位置数$a_1,a_2,\ldots, a_n$，要求为这$n$个位置数赋值，并且满足$L\leq a_i \leq R$，且序列非严格递增，问有多少种不同的赋值方案（两个方案不同当且仅当存在一个未知数被赋予不同的值），其中$n\leq 10^5, 0\leq L\leq R\leq 10^9$**
