...
connect是一个第三方模块，其实现一个请求和响应的拦截器。
...
connect的拦截器是一个函数，可以接受三个参数，第一个是请求，第二个是响应，第三个则是下一个拦截器，命名为next。
...
之后我们增加一个响应请求的拦截器，用于向客户端返回hello world。
...
如果我们交换logger和greet的use顺序，会发现logger永远不会被调用。这说明了拦截器始终是加在上一个加入的拦截器的尾部。

在使用use的时候可以额外提供一个字符串，如果字符串不是url的前缀，会跳过该拦截器，前往下一个拦截器。
...
这里我们使用了一个不存在的函数foo，因此会抛出异常。connect最终以500状态码响应请求并输出栈信息。如果你希望能手动处理异常，可以实现一个错误处理拦截器。错误处理拦截器接受四个参数：err、req、res和next。
...
在异常发生的情况下，只有异常发生的拦截器后面的错误处理拦截器会被调用，而普通拦截器会直接被跳过。
