...
\left\{
\begin{array}{rcl}
\frac{\partial }{\partial \theta^{(u)}_i}J(\ldots)&=&\sum_{j:r(u,j)=1}[(\theta^{(u)})^Tx^{(j)}-y^{(j)}]x^{(j)}_i+\lambda\theta^{(u)}_i\\
\frac{\partial }{\partial x^{(g)}_j}J(\ldots)&=&\sum_{i:r(i,g)=1}[(\theta^{(i)})^Tx^{(g)}-y^{(g)}]\theta^{(i)}_j+\lambda x^{(g)}_j\\
\end{array}
\right.
$$
