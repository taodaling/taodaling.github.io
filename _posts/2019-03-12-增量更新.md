...
```c
longestCommonPrefixLength(A, B)
{
    i = 0;
    while(i < |A| && i < |B| && A[i] == B[i])
    {
        i++;
    }
    return i;
...
display(furthest, d, i, A, B)
{
    ...
    x = -1;
    j = -1;
    if(i >= -d + 2 && furthest[d - 1][i - 1] + 1 > x)
    {
        x = furthest[d - 1][i - 1] + 1;
        j = i - 1;
    }
    if(i <= d - 2 && furthest[d - 1][i + 1] > x)
    {
        x = furthest[d - 1][i + 1];
        j = i + 1;
    }
    display(furthest, d - 1, j, A, B);
    while(x < furthest[d][i])
    {
        output(A[x]);
        x++;
    }
