...
$$
\sum_{i=0}^\infty\frac{g^{(i)}(f_0(x))}{i!}(f(x)-f_0(x))^i=0\mod x^n
$$

这里由于$f(x)-f_0(x)$的最低非零项系数最低为$\lceil\frac{n}{2}\rceil$，故有：

$$
\forall i\geq 2((f(x)-f_0(x))^i=0)
$$
...
$$
\begin{aligned}
&\sum_{i=0}^\infty\frac{g^{(i)}(f_0(x))}{i!}(f(x)-f_0(x))^i\\
=&g(f_0(x))+g'(f_0(x))(f(x)-f_0(x))\\
=&0\mod x^n\\
\Rightarrow &f(x)=f_0(x)-\frac{g(f_0(x))}{g'(f_0(x))}\mod x^n
\end{aligned}
$$
...
$$
\begin{aligned}
&f(c^j)c^{j\choose 2}=\sum_{i=0}^na_ic^{{i+j\choose 2}-{i\choose 2}}\\
