...
注意到$\{\lfloor \frac{n}{1} \rfloor\,\lfloor \frac{n}{2} \rfloor,\ldots,\lfloor \frac{n}{n} \rfloor\}$集合的大小为集合的大小为$O(\sqrt{n})$，而计算$S(n)$仅需要利用仅需要利用$\{S(\lfloor \frac{n}{1} \rfloor)\,S(\lfloor \frac{n}{2} \rfloor),\ldots,S(\lfloor \frac{n}{n} \rfloor)\}$中的值的加总。因此可以得出时间复杂度：
