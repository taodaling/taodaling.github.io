...
$$
\Big\left\lfloor \frac{\big\left\lfloor \frac{a}{b} \big\right\rfloor}{c} \Big\right\rfloor
=\Big\left\lfloor \frac{a}{bc} \Big\right\rfloor
$$

证明：

记$p_1=\Big\left\lfloor \frac{\big\left\lfloor \frac{a}{b} \big\right\rfloor}{c} \Big\right\rfloor$，那么由整除的定义我们可以得到

$$
\big\left\lfloor \frac{a}{b} \big\right\rfloor= p_1c+r_1
$$
...
因此我们认为$\Big\left\lfloor \frac{r_1b+r_2}{bc} \Big\right\rfloor = 0$。对左右两端都向下取整得到：

$$
\Big\left\lfloor \frac{a}{bc} \Big\right\rfloor=p_1+\Big\left\lfloor \frac{r_1b+r_2}{bc} \Big\right\rfloor = p_1=\Big\left\lfloor \frac{\big\left\lfloor \frac{a}{b} \big\right\rfloor}{c} \Big\right\rfloor
$$
...
$$
\Big\left\lceil \frac{\big\left\lceil \frac{a}{b} \big\right\rceil}{c} \Big\right\rceil
=\Big\left\lceil \frac{a}{bc} \Big\right\rceil
$$

记$p_1=\Big\left\lceil \frac{\big\left\lceil \frac{a}{b} \big\right\rceil}{c} \Big\right\rceil$，那么由整除的定义我们可以得到

$$
\big\left\lceil \frac{a}{b} \big\right\rceil= p_1c-r_1
$$
...
因此我们认为$\Big\left\lceil -\frac{r_1b+r_2}{bc} \Big\right\rceil = 0$。对左右两端都向上取整得到：

$$
\Big\left\lceil \frac{a}{bc} \Big\right\rceil=p_1+\Big\left\lceil-\frac{r_1b+r_2}{bc} \Big\right\rceil = p_1=\Big\left\lceil \frac{\big\left\lceil \frac{a}{b} \big\right\rceil}{c} \Big\right\rceil
$$
...
**命题：对于任意非负整数a和正整数b，c，都有$\Big\left\lceil \frac{\big\left\lfloor \frac{a}{b} \big\right\rfloor}{c}\Big\right\rceil,\Big\left\lfloor \frac{\big\left\lceil \frac{a}{b} \big\right\rceil}{c} \Big\right\rfloor \in \{\Big\left\lfloor \frac{a}{bc} \Big\right\rfloor,\Big\left\lceil \frac{a}{bc} \Big\right\rceil\}$**

证明：

考虑到$\Big\left\lfloor \frac{a}{bc} \Big\right\rfloor \leq \Big\left\lceil \frac{\big\left\lfloor \frac{a}{b} \big\right\rfloor}{c}\Big\right\rceil, \Big\left\lfloor \frac{\big\left\lceil \frac{a}{b} \big\right\rceil}{c} \Big\right\rfloor \leq \Big\left\lceil \frac{a}{bc} \Big\right\rceil$，而$\Big\left\lfloor \frac{a}{bc} \Big\right\rfloor + 1\geq \Big\left\lceil \frac{a}{bc} \Big\right\rceil$，因此命题可以直接得证。
...
$$
\sum_{i=1}^{n}f(\left\lceil{\frac{n}{i}}\right\rceil)
$$

这里不仔细讨论优化的方式，仅讨论分块的方式。

我们称$\left\lceil{\frac{n}{i}}\right\rceil$为$i$的上整值。记上整值为$x$的所有的$i$的集合$\{i_1,i_2,\ldots,i_k\}$为$x$的导出集合，记作$E(x)$。

对于任意$i$，记$j=\left\lceil\frac{n}{\left\lceil\frac{n}{i}\right\rceil}\right\rceil$，可以推出：$\left\lceil \frac{n}{i} \right\rceil < \frac{n}{j-1}$和$\left\lceil \frac{n}{i} \right\rceil \ge \frac{n}{j}$。由后者可知$j\leq \max(E(\left\lceil\frac{n}{i}\right\rceil))$，而由前者可知$\left\lceil\frac{n}{j}\right\rceil \leq \left\lceil\frac{n}{i}\right\rceil\leq \left\lceil\frac{n}{j}\right\rceil$，结合得到$j=\min(E(\left\lceil\frac{n}{i}\right\rceil))$。


$$
\min(E(x))=\left\lceil\frac{n}{x}\right\rceil\\
\max(E(x))=\left\lceil\frac{n}{x-1}\right\rceil-1
$$
...
如果想要获得$1,2,\ldots,n$的乘法逆元，如果用上面的算法，需要$O(n\log_2p)$的时间复杂度。但是利用一些简单的性质，我们就可以得到乘法逆元的递推公式，将时间复杂度优化到$O(n)$。设$k=\left\lfloor{p/x}\right\rfloor$，记$r=p(mod\phantom{1}x)$，可以得出$p=kx+r$。
...
有一个简单的结论，对于数$x=p_1^{c_1}p_2^{c_2}\ldots p_k^{c_k}$，这里$p_i$是互不相同的素数，我们定义$x$的度数为$deg(x)=c_1+c_2+\ldots+c_k$。很显然度数相同的数之间不可能存在整除关系，因此度数为某个特定数的所有数都可以组成一条反链。而n的因子中最长反链可以由所有度数正好为$\left\lfloor \frac{n}{2} \right\rfloor$的因子组成。
...
由于$a$生成的循环群
$$\{a^0,a^1,a^2,\ldots\}$$
的大小一定不会超过$p$，因此我们仅需要考虑$0\leq x\leq p-1$。我们可以利用BSGS算法来计算。记$k=\sqrt{p}$，我们提前计算$a^0,a^1,\ldots,a^k$，并将其存入哈希表中。由于假如$x$存在，那么记$i=\left\lfloor \frac{x}{k}\right\rfloor$，$j=x \mod k$，满足$x=ik+j$。代入公式得到$a^{ik+j}=b\Rightarrow a^{j}=ba^{-ik}$。我们可以直接算出$ba^{-ik}$（由于$a$与$k$互质，可以利用扩欧得到$a^{-1}$）并拿到哈希表中查找是否出现即可。
...
设$a$为非负整数，$b$为正整数，那么一定有$a=kb+c$，其中$0\leq c<b$，$k=\left\lfloor\frac{a}{b}\right\rfloor$。我们记$c=a\mod b$。
