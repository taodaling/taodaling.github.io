...
如果想要获得$1,2,\ldots,n$的乘法逆元，如果用上面的算法，需要$O(n\log_2p)$的时间复杂度。但是利用一些简单的性质，我们就可以得到乘法逆元的递推公式，将时间复杂度优化到$O(n)$。设$k=\left\lfloor{p/x}\right\rfloor$，记$r=p(\mod x)$，可以得出$p=kx+r$。


$$
p=kx+r\\
\rightarrow kx+r=0(\mod p)\\
\rightarrow r^{-1}x+k^{-1}=0(\mod p)\\
\rightarrow r^{-1}+x^{-1}k^{-1}=0(\mod p)\\
\rightarrow x^{-1}=-kr^{-1}(\mod p)\\
$$
